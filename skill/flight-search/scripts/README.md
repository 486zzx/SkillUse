...
- **可选**：`--arrival-time "yyyy-MM-dd HH:mm" "yyyy-MM-dd HH:mm"`、`--max-price 2000`、`--sort-by price_asc`。
- **输出**：成功为 `{"success":true,"result":{"flightInfo":[...],"flightCount":N}}`；失败为 `{"success":false,"error":"..."}`；参数不足为 `{"success":false,"clarification_needed":true,"missing":[...],"message":"..."}`。
...
**脚本**：`normalize_date.py`（供多段解析等子流程使用）
