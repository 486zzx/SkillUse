...
from bm25_ngram import score_documents_with_bm25_ngrams
...
def bm25_prefilter(items: list[dict], keywords: list[str] | None) -> list[dict]:
    ...
    keep_top = set(i for i, _ in indexed[: _config.BM25_MIN_KEEP])
    out: list[dict] = []
    for i, item in enumerate(items):
        s = float(scores[i])
        if s >= _config.BM25_MIN_SCORE or i in keep_top:
            row = dict(item)
