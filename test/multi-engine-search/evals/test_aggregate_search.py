...
# ---------------------------------------------------------------------------
# 5.1 query 与 keywords 参数
# ---------------------------------------------------------------------------
def test_M_QK_03_no_query_no_k():
    """无 query 无 k：应报错。"""
    data = _run_aggregate_search("")
    assert data.get("success") is False
    assert "error" in data
    assert "query" in (data.get("error") or "").lower() or "k" in (data.get("error") or "").lower() or "关键词" in (data.get("error") or "")


@requires_api_key
def test_M_QK_01_query_only():
    data = _run_aggregate_search("python是什么")
...
@requires_api_key
def test_M_QK_02_query_with_k():
    data = _run_aggregate_search("Rust 和 Go 性能对比", "-k", "Rust Go 性能", "-k", "Go 2024")
    assert data.get("success") is True
    assert "results" in data
...
def test_M_OUT_03_max_items():
    data = _run_aggregate_search("python 教程")
    assert data.get("success") is True
...
def test_M_OUT_07_no_site():
    data = _run_aggregate_search("python 是什么")
    assert data.get("success") is True
